...
class AscendexDataFeed {
  ...
  async getCandlesData(base, quote, resolution, startTime, endTime) {
    let endpointPath = `/tradingview/ascendex?symbol=${base.toUpperCase()}/${quote.toUpperCase()}&interval=${resolution}&from=${startTime}`;
    if (endTime) {
  ...
  getBars(symbolData, resolution, periodParams, onHistoryCallback, onErrorCallback) {
    const { from: startDate, to: endDate, countBack, firstDataRequest } = periodParams;
    this.allCandles = [];
    ...
    console.log(
      ...
      endDate,
      dayjs.unix(endDate),
      dayjs.unix(endDate).startOf("minute"),
    ...
    this.getCandlesData(
      ...
      .then((newCandles) => {
        // Accumulate candles.
        this.allCandles = this.allCandles.concat(newCandles.data);
