...
const DCAPanel = (props) => {
  ...
  }, [expanded, rebuyTargets, pricePriority]);

  const resetPriorityErrors = () => {
  ...
  useEffect(resetPriorityErrors, [pricePriority]);
    ...
    return (
      <Box className="targetGroup" data-target-id={targetId} key={`target${targetId}`}>
        <Box className="targetPrice" display="flex" flexDirection="row" flexWrap="wrap">
          ...
          />
          <Box alignItems="center" display={pricePriority === "price" ? "flex" : "none"}>
            <OutlinedInput
