...
function getStorage(mode: "local" | "session") {
  switch (mode) {
    ...
    default: {
      throw new Error(`Invalid mode ${mode}, it must be either local or session.`);
    }
  }
}
...
export function syncWithStorage(mode: "local" | "session", parser = baseParser) {
  ...
  return cache.subscribe(() => {
    ...
    for (let key of keys) {
      storage.setItem(`swr-${key}`, JSON.stringify({ swrValue: cache.get(key) }));
    }
  });
}
